...
Inductive bin : Type :=
  | Zero : bin
  | Double : bin -> bin
  | DoubleAndOne : bin -> bin.

Fixpoint inc_bin (b : bin) : bin :=
  match b with
  | Zero => DoubleAndOne Zero
  | Double b' => DoubleAndOne b'
  | DoubleAndOne b' => Double (inc_bin b')
  end.

Fixpoint bin_to_nat (b : bin) : nat :=
  match b with
  | Zero => O
  | Double b' => mult 2 (bin_to_nat b')
  | DoubleAndOne b' => S (mult 2 (bin_to_nat b'))
  end.

Example inc_three_four: (bin_to_nat (inc_bin (DoubleAndOne (DoubleAndOne Zero)))) = 4.
Proof. reflexivity. Qed.

Example inc_nine_ten: (bin_to_nat (inc_bin (DoubleAndOne (Double (Double (DoubleAndOne Zero)))))) = 10.
Proof. reflexivity. Qed.

Example zero_is_zero: (bin_to_nat Zero) = 0.
Proof. reflexivity. Qed.

Example five_is_five: (bin_to_nat (DoubleAndOne (Double (DoubleAndOne Zero)))) = 5.
Proof. reflexivity. Qed.
(** [] *)
