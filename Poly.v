...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => andb (negb (blt_nat n 7)) (evenb n)) l.
...
Theorem dist_map_snoc : forall (X Y : Type) (f : X -> Y) (h : X) (t : list X),
...
Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
  fold (fun h t => f h :: t) l nil.

(** Write down a theorem in Coq stating that [fold_map] is correct,
    and prove it. *)

Theorem fold_map_is_map : forall (X Y : Type) (f : X -> Y) (l : list X), fold_map f l = map f l.
Proof.
  intros.
  induction l as [|h t].
  reflexivity.
