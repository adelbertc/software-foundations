...
Theorem add_is_count_plus_one : forall (n : nat) (b : bag), count n (add n b) = S (count n b).
Proof. Admitted.
...
Proof.
  ...
  Case "l1 = nil".
    reflexivity.
  Case "l1 = cons". (* S (length l1') + length l2  -->  S (length l1' + length l2) *)
    simpl. rewrite -> IHl1'. reflexivity.  Qed.
...
Proof.
  ...
  Case "l = nil".
    reflexivity.
  Case "l = cons n' l'". (* length (n' :: snoc l' n) --> S (length (snoc l' n) *)
...
Theorem rev_snoc_cons : forall (n : nat) (l : natlist), rev (snoc l n) = n :: rev l.
Proof.
  intros n l.
  induction l as [|h t].
  reflexivity.
...
Theorem beq_nat_succ : forall (n : nat) (l : natlist), true = beq_natlist (n :: l) (n :: l) -> true = beq_natlist (S n :: l) (S n :: l).
Proof.
  intros n l.
  intros H.
  Abort.
...
Theorem list_design : forall (n : nat) (l : natlist), l ++ n :: l = snoc l n ++ l.
Proof.
  intros n l.
  induction l as [|h t].
  reflexivity.
...
Theorem count_nil : forall n : nat, count n [] = 0.
Proof. reflexivity. Qed.
  

Theorem plus_right_identity : forall n : nat, n + 0 = n.
Proof.
  induction n as [|n'].
  reflexivity.
  simpl.
...
Theorem bag_count_sum : forall (n : nat) (s t : natlist), count n s + count n t = count n (sum s t).
Proof.
  intros n s t.
  induction s as [|x xs].
  reflexivity.
