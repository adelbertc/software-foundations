...
Proof.
  ...
  Case "Proof of assertion - associativity".
  rewrite -> plus_assoc. reflexivity.
  rewrite -> H.
  assert (H': n + m = m + n).
  Case "Proof of assertion - commutativity".
  rewrite -> plus_comm. reflexivity.
  rewrite -> H'.
  rewrite -> plus_assoc.
  reflexivity.
...
Theorem mult_m_O : forall m : nat, m * O = O.
Proof.
  intros m.
  induction m as [|m'].
  reflexivity.
...
Theorem mult_S_dist : forall n m : nat, n * S m = n + n * m.
Proof.
  intros n m.
  induction n as [|n'].
  Case "n = 0".
...
Theorem all3_spec : forall b c : bool, (* (b and c) or (-b or -c) = true  *)
    orb
      (andb b c)
      (orb (negb b)
               (negb c))
...
Fixpoint bin_to_nat (b : bin) : nat :=
  ...
  | DoubleAndOne b' => 1 + (2 * (bin_to_nat b'))
  end.

Theorem s_n_plus_one : forall n : nat, S n = n + 1.
Proof.
  intros n.
  induction n as [|n'].
  reflexivity.
...
Theorem bin_comm : forall b : bin, bin_to_nat (inc_bin b) = (bin_to_nat b) + 1.
Proof.
  intros b.
  induction b as [|b'|b''].
  reflexivity.
  simpl.
  rewrite -> plus_0_r.
  rewrite -> s_n_plus_one.
  reflexivity.
  simpl.
  rewrite -> plus_0_r.
  rewrite -> IHb''.
  replace (S (bin_to_nat b'' + (bin_to_nat b'' + 0) + 1)) with ((bin_to_nat b'' + (bin_to_nat b'' + 0) + 1) + 1).
  rewrite -> plus_0_r.
  rewrite -> plus_swap.
  rewrite -> plus_assoc.
  rewrite -> plus_assoc.
...
Theorem nat_round_trip : forall n : nat, bin_to_nat (nat_to_bin n) = n.
Proof.
  intros n.
  induction n as [|n'].
  reflexivity.
...
Fixpoint is_zero (b : bin) : bool :=
  ...
  | Zero => true
  | Double b' => is_zero b'
  | DoubleAndOne _ => false
  end.

Fixpoint normalize (b : bin) : bin :=
  match b with
  | Zero => Zero
  | Double b' => match (is_zero b') with
                 | true => Zero
                 | false => Double (normalize b')
                 end
  | DoubleAndOne b' => DoubleAndOne (normalize b')
  end.

Theorem bin_round_trip : forall b : bin, nat_to_bin (bin_to_nat b) = normalize b.
Proof.
  intros b.
  induction b as [|b'|b''].
  Case "b = 0".
